...
def main():
    ...
    fFar = 1000.0
    fFov = 90.0
    fFovRad = 1 / (math.tan(fFov * 0.5 / 180.0 * pi))
    ...
    PROJECTION_MATRIX = [[0]*4 for _ in range(4)] 
    ...
    # Create a new vector which holds the coordinates of our triangles in the mesh, but projected.
    for tri in meshCube.tris:
        triOnScreen = Triangle(Vector(0,0,0), Vector(0,0,0), Vector(0,0,0))
        matrixMultVector(tri.pts[0], triOnScreen.pts[0], PROJECTION_MATRIX)        
        matrixMultVector(tri.pts[1], triOnScreen.pts[1], PROJECTION_MATRIX)
        matrixMultVector(tri.pts[2], triOnScreen.pts[2], PROJECTION_MATRIX)
        ...
        triOnScreen.pts[0].x *= 0.5 * WIDTH
        triOnScreen.pts[0].y *= 0.5 * HEIGHT
        triOnScreen.pts[1].x *= 0.5 * WIDTH
        triOnScreen.pts[1].y *= 0.5 * HEIGHT
        triOnScreen.pts[2].x *= 0.5 * WIDTH
        triOnScreen.pts[2].y *= 0.5 * HEIGHT
